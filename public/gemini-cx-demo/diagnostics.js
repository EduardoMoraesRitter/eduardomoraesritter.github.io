...
(function (root) {
  function codes(detail = {}) {
    ...
    const http = numeric(detail.status);
    return {
      ...(http >= 100 && http <= 599 ? { http } : {}),
      ...(numeric(detail.errorCode) !== undefined ? { code: numeric(detail.errorCode) } : {}),
      ...(numeric(detail.detailedErrorCode) !== undefined ? { detail: numeric(detail.detailedErrorCode) } : {}),
    };
  }
  ...
  function describe(detail = {}) {
    ...
    if (values.http === 429 || values.code === 21) return result('capacity', 'Limite de pedidos ou capacidade', 'O Google recusou este pedido por limite de utilização ou capacidade. Aguarda antes de tentar novamente. O detalhe da quota deve ser consultado no projeto Google.');
