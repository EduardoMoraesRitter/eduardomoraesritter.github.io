import {controlKey,digest,seal,open} from '../farol4/protocol.mjs?v=20260920-1';
...
export class ReturnChannel {
  constructor(onMessage,onState,createClient) {
    ...
    this.outbox=Promise.resolve();this.inbox=Promise.resolve();
  ...
  async connect(url,key,code) {
    ...
    this.channel.on('broadcast',{event:'control'},({payload})=>{
      ...
      try {
        ...
        if(generation!==this.generation||m.v!==4||typeof m.from!=='string'||m.from.length>64||m.from===this.identity||
          ...
          !['missing','done','hello','hello_ack','ready','pause_state','pause_ack','reset','reset_ack','sync_request','sync_offer','sync_ready','sync_ack'].includes(m.type))return;
        if(m.seq<=(this.peers.get(m.from)||0))return;
        if(this.peers.size>32&&!this.peers.has(m.from))return;
        this.peers.set(m.from,m.seq);await this.onMessage(m);
  ...
  send(message) {
    ...
    const job=this.outbox.then(async()=>{
      if(generation!==this.generation||!this.ready||!this.channel)return false;
      const channel=this.channel;
      const payload=await seal(this.key,{...message,v:4,from:this.identity,seq:++this.sequence,time:Date.now()});
      if(generation!==this.generation)return false;
      return await channel.send({type:'broadcast',event:'control',payload})==='ok';
    }).catch(()=>false);
