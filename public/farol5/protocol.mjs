// Farol 5: numbered RGB blocks with acoustic feedback.
...
export function validMeta(m) {
  return !!m && m.v===5 && /^[a-f0-9]{64}$/.test(m.hash) &&
...
export class Sender {
  static async create(bytes,name,bs=400) {
    ...
    const meta={v:5,id:`${hash}:${bs}`,hash,name:name.slice(0,180),size:bytes.length,bs,total:Math.max(1,Math.ceil(bytes.length/bs))};
  ...
  metadata() { return 'F5|M|'+b64(enc.encode(JSON.stringify(this.meta))); }
  packet(i) {
    ...
    return `F5|B|${this.meta.id}|${i}|${crc32(data)}|${b64(data)}`;
  }
...
export class Receiver {
  ...
  accept(packet) {
    ...
    if(p.length!==6||p[0]!=='F5'||p[1]!=='B'||p[2]!==this.meta.id||!/^\d+$/.test(p[3])) return false;
...
export function readMeta(packet) {
  if(typeof packet!=='string'||!packet.startsWith('F5|M|')||packet.length>1600) return null;
