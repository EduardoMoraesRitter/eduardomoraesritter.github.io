import {readProductionProgress} from './progress-import.mjs?v=staging-20260909-1';
import {cameraConstraints,syncCameraAspect} from './camera-view.mjs?v=staging-20260909-1';
import {Sender,Receiver,readMeta,ranges,parseRanges,newPairCode,MAX_BYTES} from './protocol.mjs?v=staging-20260909-1';
import {ReturnChannel} from './realtime.mjs?v=staging-20260909-1';
import {analyzeFrame,autoZoom} from './camera.mjs?v=staging-20260909-1';
import {pairPacket,readPair} from './pairing.mjs?v=staging-20260909-1';
import {TransferRate,duration} from './transfer-rate.mjs?v=staging-20260909-1';
import {PauseState} from './pause-state.mjs?v=staging-20260909-1';
import {readSession,writeSession} from './session-store.mjs?v=staging-20260909-1';
import {diagnoseTransfer} from './diagnostics.mjs?v=staging-20260909-1';
import {createCalibration} from './calibration.mjs?v=staging-20260909-1';
...
function forgetRoom(){
  try{localStorage.removeItem('farol4-staging-session-'+mode);}catch{}
...
function setMode(value){calibration.cancel();if(mode!==value)restoredRoom=null;if(mode!==value&&transferId())setPaused(true);mode=value;try{sessionStorage.setItem('farol4-staging-role',mode);}catch{}stop();stopCamera();$('sendView').hidden=value!=='send';$('receiveView').hidden=value!=='receive';$('createPair').hidden=value!=='send';$('joinPair').hidden=value!=='receive';$('connectionDetails').open=false;$('sendMode').setAttribute('aria-pressed',String(value==='send'));$('receiveMode').setAttribute('aria-pressed',String(value==='receive'));document.body.dataset.mode=value;feedback();updatePauseUI();}
...
const importProgress=document.createElement('button');importProgress.id='importProgress';importProgress.className='textButton';importProgress.textContent='Copiar progresso da versão principal';$('discard').after(importProgress);
importProgress.onclick=async()=>{
  if(mode!=='receive')return;
  if(receiver&&!confirm('Substituir somente o progresso deste staging pela cópia da versão principal?'))return;
  try{
    const snapshot=await readProductionProgress();if(!snapshot){notice('Nenhum progresso da versão principal foi encontrado neste navegador.');return;}
    const candidate=Receiver.restore(snapshot);
    if(transferId())setPaused(true);stopCamera();forgetRoom();await connection.close();onConnectionState('CLOSED');
    receiver=candidate;pairTarget=null;lastPairCode='';$('pairCode').value='';pauseState.reset();storageTouched=true;dirty=true;
    await persist();updateReceiver();notice(`Copiados ${receiver.count} de ${receiver.meta.total} blocos. A versão principal foi preservada. Leia o QR deste staging para conectar.`);
    if(receiver.count===receiver.meta.total)verifyReceiver();
  }catch(e){notice('Não foi possível copiar o progresso: '+e.message);}
};
async function openStore(){
  return new Promise(resolve=>{
    const request=indexedDB.open('farol4-staging',1);request.onupgradeneeded=()=>request.result.createObjectStore('sessions');
...
async function connect(create){
  ...
  try{
    ...
    try{localStorage.setItem('farol4-staging-config',JSON.stringify(config));}catch{}
...
async function initialize(){
  let savedRole;try{savedRole=sessionStorage.getItem('farol4-staging-role');}catch{}
  ...
  try{config=JSON.parse(localStorage.getItem('farol4-staging-config')||'null')||config;}catch{}
