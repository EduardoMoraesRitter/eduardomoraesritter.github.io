# Staging — câmera e retomada por blocos faltantes

Esta cópia de teste usa `/farol4-staging/`. Abra esse endereço nos dois aparelhos; salas, configuração e progresso são separados da versão principal. Em Recuperação manual do receptor, **Copiar progresso da versão principal** importa uma cópia dos blocos desse mesmo navegador, sem apagar o original. Depois leia o QR do transmissor do staging com o mesmo arquivo e tamanho de bloco.

No celular, a solicitação da câmera acompanha a orientação e a prévia mantém a proporção real informada pelo aparelho, sem recorte de preenchimento. Em formato alto pode ser necessário rolar. O contador acima da câmera distingue blocos novos, repetidos e inválidos. O hardware ainda pode escolher outra resolução; câmera real precisa ser validada.

No desktop o QR ganha área, com controles à direita. Ao conectar, o receptor informa progresso mesmo pausado; o primeiro envio prioriza apenas os faltantes conhecidos. Quando o lote termina, o transmissor aguarda outro pedido em vez de voltar ao início. Se os primeiros 900 blocos estiverem salvos, o próximo solicitado será o901; se houver lacunas anteriores, elas também são pedidas. Relatórios periódicos podem repetir um bloco recebido desde o último relatório, mas não reiniciam o arquivo inteiro.

Validação: teste unitário com3000blocos e duaslacunas; teste real Supabase com25%restaurado e primeiro envio3,4,5; pausa/reconexão/download/restauração completos; cópia de progresso preserva original; quatro combinações de orientação/fonte de câmera sintética. Câmera física não testada.
