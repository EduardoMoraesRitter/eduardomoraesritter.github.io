...
export async function videoPrints(file,{interval=1,png=false,signal,onProgress=()=>{}}={}){
 ...
 try{
  ...
  const context=canvas.getContext('2d');let size=0;
  for(let i=0;i<count;i++){
   ...
   onProgress(`Capturando ${i+1} de ${count} prints · ${(size/1e6).toFixed(1)} MB`);
   await new Promise(resolve=>setTimeout(resolve,0));
  }
  entries.push({name:'LEIA-ME.json',blob:new Blob([JSON.stringify({video:file.name,interval_seconds:interval,duration_seconds:duration,timestamps_seconds:times,note:'Capturas sem áudio; quadros intermediários não incluídos.'},null,2)])});
