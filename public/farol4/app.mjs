import {Sender,Receiver,readMeta,ranges,parseRanges,newPairCode,MAX_BYTES} from './protocol.mjs?v=20260909-2';
import {ReturnChannel} from './realtime.mjs?v=20260909-2';
import {analyzeFrame,autoZoom} from './camera.mjs?v=20260909-2';
import {pairPacket,readPair} from './pairing.mjs?v=20260909-2';
import {TransferRate,duration} from './transfer-rate.mjs?v=20260909-2';
import {PauseState} from './pause-state.mjs?v=20260909-2';
import {readSession,writeSession} from './session-store.mjs?v=20260909-2';
import {diagnoseTransfer} from './diagnostics.mjs?v=20260909-2';
import {createCalibration} from './calibration.mjs?v=20260909-2';
...
function receivePacket(packet){
  ...
  if(receiver&&!receiver.verified&&!pauseState.value.paused){
    if(receiver.accept(packet)){lastProgressAt=performance.now();dirty=true;updateReceiver();if(receiver.count===receiver.meta.total)verifyReceiver();}
  }
}
