...
export function readSession(storage,role,now=Date.now()){
  try{const key='farol4-session-'+role,value=validSession(JSON.parse(storage.getItem(key)),now);
...
export function writeSession(storage,value){
  ...
  try{storage.setItem('farol4-session-'+safe.role,JSON.stringify(safe));return true;}catch{return false;}
}
