async page=>{
 ...
 await s.locator('#fps').fill('2');await frame();await s.waitForFunction(()=>window.farol3Engine.state().sending);
 ...
 await r.waitForFunction(()=>window.farol3Engine.state().camera);await s.waitForTimeout(1000);
 if(await s.evaluate(()=>window.farol3Engine.state().sending))throw Error('Resumed before optical confirmation');
 await frame();await s.waitForFunction(()=>window.farol3Engine.state().sending);
