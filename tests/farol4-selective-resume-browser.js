...
async page=>{
 ...
 const saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('farol4-session-send')));
 ...
 await r.evaluate(async saved=>{
  const {Sender,Receiver}=await import('./protocol.mjs?v=20260909-3');
  ...
  const db=await new Promise((resolve,reject)=>{const q=indexedDB.open('farol4',1);q.onsuccess=()=>resolve(q.result);q.onerror=()=>reject(q.error);});
  ...
  const {writeSession}=await import('./session-store.mjs?v=20260909-3');writeSession(localStorage,{...saved,role:'receive',pause:{revision:5,paused:true,author:'restore-test'}});sessionStorage.setItem('farol4-role','receive');
 },saved);
