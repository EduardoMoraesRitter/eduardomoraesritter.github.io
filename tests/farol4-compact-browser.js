async page=>{
 ...
 await p.locator('#printsCompact').selectOption('compact');await p.locator('#previewPrints').click();
 await p.waitForFunction(()=>!document.querySelector('#printsPreview').hidden);
 if(!(await p.locator('#previewSize').textContent()).includes('1600×900'))throw Error('Preview not resized');
