async page=>{
 ...
 await p.route('**/farol4/app.mjs*',async route=>{const res=await route.fetch();await route.fulfill({response:res,body:await res.text()+`\nwindow.__test={receivePacket,unpause:()=>pauseState.reset(),state:()=>({receiver:receiver?.meta.id,count:receiver?.count,pairTarget,room:connection.roomId}),seedRoom:()=>{restoredRoom={file:'a'.repeat(64)+':400',cursor:0};}};`});});
 ...
 await p.locator('#receiveMode').click();if(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Overflow');await p.screenshot({path:'output/playwright/farol4-new-reception.png',fullPage:true});await p.evaluate(async()=>{const {Sender}=await import('/farol4/protocol.mjs');const a=await Sender.create(new Uint8Array(800).fill(3),'reset.bin',400);window.__test.unpause();window.__test.receivePacket(a.metadata());window.__test.receivePacket(a.packet(0));localStorage.setItem('another-project-test','keep');localStorage.setItem('farol4-test','remove');});
 await p.waitForTimeout(2700);p.once('dialog',d=>d.dismiss());await p.locator('#resetAll').click();if(await p.evaluate(()=>window.__test.state().count)!==1)throw Error('Cancelled reset lost data');
 p.once('dialog',d=>d.accept());await Promise.all([p.waitForURL('**/*reset=*'),p.locator('#resetAll').click()]);await p.waitForTimeout(500);
 if(await p.evaluate(()=>window.__test.state().receiver))throw Error('Reset restored previous file');
 if(await p.evaluate(()=>localStorage.getItem('farol4-test')!==null||localStorage.getItem('another-project-test')!=='keep'))throw Error('Reset storage scope wrong');
 await p.screenshot({path:'output/playwright/farol4-reset-mobile.png',fullPage:true});await context.close();
}
